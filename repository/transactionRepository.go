...
func (r *transactionRepository) FindByBankAccountId(bankAccountId int) ([]entity.Transaction, error) {
	...
	err := r.db.Where("bank_account_id = ? AND deleted_by = ?, AND created_date = DATE_SUB(CURDATE(), INTERVAL 1 WEEK)", bankAccountId, "").Find(&transaction).Error
...
func (r *transactionRepository) FindByUserId(id int) ([]entity.Transaction, error) {
	...
	err := r.db.Table("transactions").Joins("JOIN bank_accounts on transactions.bank_account_id = bank_accounts.id").Where("bank_accounts.account_id_owner = ? AND transactions.deleted_by = ?", id, "").Preload("BankAccount").Find(&transaction).Error
	if err != nil {
		return transaction, err
	}
