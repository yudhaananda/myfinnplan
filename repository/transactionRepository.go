...
func (r *transactionRepository) FindByUserId(id int) ([]entity.Transaction, error) {
	...
	err := r.db.Table("transactions").Joins("JOIN bank_accounts on transactions.bank_account_id = bank_accounts.id").Where("bank_accounts.account_id_owner = ? AND transactions.deleted_by = ?", id, "").Preload("BankAccount").Preload("UserAccount").Find(&transaction).Error
	if err != nil {
		return transaction, err
	}
